...
type SurveyState = 'consent' | 'questions' | 'confirm' | 'done'
...
export default function SurveyPage() {
  ...
  const [state, setState] = useState<SurveyState>('consent')
  ...
  useEffect(() => {
    async function loadAssessment() {
      ...
      const info = data as unknown as AssessmentInfo
      if (info.status !== 'coletando') {
        setError('Esta avaliacao nao esta recebendo respostas no momento.')
        setLoading(false)
        return
      }
  ...
  async function loadQuestions() {
    ...
    const { data: factorsData } = await sb
      .from('factors')
      .select('*')
    ...
    if (!factorsData || !questionsData) {
      setError('Erro ao carregar questionario.')
      return
    }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const { error: sessaoError } = await sb.from('sessao_anonima').insert({
        token: sessionToken,
      ...
      if (sessaoError) {
        setError('Erro ao criar sessao. Tente novamente.')
        setSubmitting(false)
        return
      }
    ...
    return (
      <div className="survey-container">
        <div className="survey-header">
          <h1>Avaliacao de Riscos Psicossociais</h1>
          <p>NR-1 — Instrumento de coleta anonima</p>
        </div>
